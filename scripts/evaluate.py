...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--algorithm", "--algo", choices=list(list_algorithms()), default="sac")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--benchmark", choices=list(list_benchmarks()), default="lane_following_v0")
    parser.add_argument("--episodes", type=int, default=0, help="0 uses all benchmark seeds")
    parser.add_argument("--port", type=int, default=Config.port)
    parser.add_argument("--network", choices=["SAC", "Attention_SAC"], default=Config.network)
    parser.add_argument("--logger", dest="logger_backend", choices=["tensorboard", "wandb", "both", "none"], default="tensorboard")
...
def main() -> None:
    ...
    seeds = benchmark.seeds[:args.episodes] if args.episodes > 0 else benchmark.seeds
    ...
    adapter = VectorObservationAdapter(cfg.state_dim, risk_field_dim=cfg.risk_field_sectors)

    try:
        report = BenchmarkEvaluator(logger).evaluate(
            benchmark.name, env, agent, adapter, seeds
        )
