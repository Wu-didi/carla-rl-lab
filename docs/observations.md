...
| --- | --- | --- | --- |
| `image` | `(9, 84, 84)` | uint8 `[0, 255]` | Three chronological front RGB frames, channel-first |
...
The camera is mounted at `(x=1.5 m, z=2.4 m)` with a 90-degree field of view.
CARLA runs at 10 Hz (`dt=0.1`) and the sensor produces one frame per world
tick. At reset, the first frame is repeated until the temporal stack is full.
...
```text
3 * 3 * 84 * 84 + 10 * 2 + 2 = 63,526 uint8 values
```
...
Increasing `image_size` to 256 produces a packed state of 589,846 bytes and
